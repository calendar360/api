...
export const kingschatProfileLogin = async (req, res) => {
  try {
    ...
    if (!kcId) {
      return res.status(400).json({ success: false, message: 'KingsChat profile missing id' });
    }

    const fullName = kcUser.name || kcUser.fullName || 'KingsChat User';
    const emailRaw = data.email ?? kcUser.email;
    const email =
      (typeof emailRaw === 'object' && emailRaw !== null ? emailRaw.address : emailRaw) ||
      `kc_${kcId}@kingschat.local`;
...
export const kingschatLogin = async (req, res) => {
  const { accessToken, refreshToken: kcRefreshToken } = req.body || {};
  ...
  try {
    ...
    const kcId = String(profile?.id || profile?.userId || kcUser?.user_id || kcUser?.id || '');
    if (!kcId) {
      return res.status(400).json({ success: false, message: 'KingsChat profile missing id' });
    }

    const fullName = kcUser?.name || profile?.name || profile?.fullName || 'KingsChat User';
    const emailRaw = profile?.email ?? kcUser?.email;
    const email =
      (typeof emailRaw === 'object' && emailRaw !== null ? emailRaw.address : emailRaw) ||
      `kc_${kcId}@kingschat.local`;
    const avatar = kcUser?.avatar_url || profile?.avatar || profile?.picture || null;
    const username = kcUser?.username || String(email).split('@')[0];
    ...
    const message =
      error.response?.data?.message || error.message || 'KingsChat login failed';
